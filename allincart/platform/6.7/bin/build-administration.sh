...
if [[ -e "${PROJECT_ROOT}/vendor/allincart/platform" ]]; then
    ADMIN_ROOT="${ADMIN_ROOT:-"${PROJECT_ROOT}/vendor/allincart/platform/src/Administration"}"
else
    ADMIN_ROOT="${ADMIN_ROOT:-"${PROJECT_ROOT}/vendor/allincart/administration"}"
fi
...
# build admin
[[ ${ALLINCART_SKIP_BUNDLE_DUMP:-""} ]] || "${BIN_TOOL}" bundle:dump
...
# Dump entity schema
if [[ -z "${ALLINCART_SKIP_ENTITY_SCHEMA_DUMP:-""}" ]] && [[ -f "${ADMIN_ROOT}"/Resources/app/administration/scripts/entitySchemaConverter/entity-schema-converter.ts ]]; then
...
[[ ${ALLINCART_SKIP_ASSET_COPY:-""} ]] ||"${BIN_TOOL}" assets:install
